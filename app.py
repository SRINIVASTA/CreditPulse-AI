    ...
    if uploaded_file is not None:
        try:
            if uploaded_file.name.endswith('.csv'):
                ...
                # header=1 is required for the UCI Credit Card Excel file
                return pd.read_excel(uploaded_file, header=1)
        except Exception as e:
            st.error(f"Error reading file: {e}")
            return None
    else:
        # Fallback Mock Data so the app isn't empty on first load
        data_size = 500
        return pd.DataFrame({
            'ID': range(1, data_size + 1),
            'LIMIT_BAL': np.random.choice([5000, 10000, 20000], data_size),
...
if df is not None:
    ...
    if all(col in df.columns for col in required):
        ...
        # Handle case where BILL_AMT2 might be missing
        ...
        m3.metric("Growth Opps", len(df[df['Autonomous_Action'] == "🌟 GROWTH"]))
        ...
        with c1:
            st.subheader("Portfolio Action Distribution")
            counts = df['Autonomous_Action'].value_counts()
            st.bar_chart(counts, color="#3498DB")
        ...
        st.download_button("📥 Download Action Report", csv, "risk_report.csv", "text/csv")
        
    else:
        st.error("⚠️ Column Mismatch!")
        st.write("The system expects: `BILL_AMT1`, `LIMIT_BAL`, and `PAY_0`.")
